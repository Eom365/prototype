...
import { composeAddress } from '../cardScope'
import './Stage13.css'
...
function readCard(product) {
    ...
    const points = (product.shipmentPoints || []).filter((item) => !item.variationId)
    const point = points.find((item) => item.active && item.quantity)
        || points.find((item) => item.quantity)
        || points.find((item) => item.active)
        || points[0]
    const article = (product.values || []).find((value) => value.code === 'article' && !value.variationId)
    const address = point
        ? (point.addressLine || composeAddress({
            name: point.name || '',
...
function variationPoint(product, variation) {
    const points = (product?.shipmentPoints || []).filter((item) => item.variationId === variation?.id)
    return points.find((item) => item.active && item.quantity)
        || points.find((item) => item.quantity)
        || points.find((item) => item.active)
        || null
}
...
function pointAddress(point) {
    if (!point) return ''
    return point.addressLine || composeAddress({
        name: point.name || '',
        index: point.postalCode || '',
        region: point.region || '',
        city: point.city || '',
...
function Stage13() {
    ...
    const handleCreateVariant = async () => {
        ...
        try {
            ...
            setBundle((prev) => prev ? { ...prev, variations: [...(prev.variations || []), created] } : prev)
        ...
        <>
            ...
            <div className="variants-section">
                                        ...
                                        <>
                                            ...
                                            {card.quantity && (
                                                <p className="product-card__quantity">
                                                    Количество - {card.quantity} штуки.
                                                </p>
                                            )}
                                        </>
                                    ) : (
                                ...
                                <div className="variant-card__sections">
                                    ...
                                    <div className="variant-card__col">
                                        ...
                                        <button type="button" className="variant-card__section variant-card__section--pending" onClick={() => goBase('/stage10')}>
                                            ...
                                            {card?.address && <span className="variant-card__address">{card.address}</span>}
                                        </button>
                ...
                {savedCards.map((variation) => {
                    ...
                    const stock = variationPoint(bundle, variation)
                    const address = pointAddress(stock)
                    ...
                    <div className="product-card" key={variation.id}>
                        ...
                        <div className="product-card__body">
                            ...
                            <PhotoStrip photos={(bundle.files || []).filter((file) => file.variationId === variation.id && (file.role === 'presentation' || file.role === 'product'))} />
                            <div className="product-card__col product-card__col--info">
                            ...
                            <div className="product-card__col product-card__col--address">
                                {address ? (
                                    <>
                                        ...
                                        {stock?.quantity && (
                                            <p className="product-card__quantity">
                                                Количество - {stock.quantity} штуки.
                                            </p>
                                        )}
                                    </>
                                ) : (
                            ...
                            <div className="variant-card__sections">
                                <div className="variant-card__col">
                                    <button
                                        ...
                                        onClick={() => go('/stage16', currentVariation.id)}
                                    >
                                        Наименование и категория <span className="variant-card__arrow">›</span>
                                    </button>
                                    <button
