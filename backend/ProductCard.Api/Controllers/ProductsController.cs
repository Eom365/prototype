...
public class ProductsController : ControllerBase
{
    ...
    public async Task<ActionResult<ProductDetailDto>> SaveCategory(Guid id, CategoryDto dto)
    {
        ...
        product.Purpose = string.IsNullOrWhiteSpace(dto.Purpose) ? "Стоматология" : dto.Purpose;
        product.ProductKindId = kind?.Id;
        product.ProductName = dto.ProductName;
        product.CategoryPath = string.IsNullOrWhiteSpace(dto.CategoryPath) && kind != null
            ? $"Стоматология > {kind.Category.Name} > {kind.Name}"
            : dto.CategoryPath;
    ...
    public async Task<ActionResult<VariationDto>> CreateVariation(Guid id, VariationWriteDto dto)
    {
        ...
        var baseValues = product.Values.Where(item => item.VariationId == null).ToList();
        foreach (var value in baseValues)
        {
            copied.Add(value.Code);
            _db.CharacteristicValues.Add(CopyValue(id, variation.Id, value, dto));
        }

        if (!copied.Contains("article"))
            AddCode(id, variation.Id, "article", dto.Article);
        if (!copied.Contains("model"))
        ...
        if (dto.Values != null)
        {
            ...
            {
                var current = _db.ChangeTracker.Entries<CharacteristicValue>()
                    .Select(entry => entry.Entity)
                    .FirstOrDefault(item => item.VariationId == variation.Id && item.Code == incoming.Code);
                if (current == null)
                {
                    AddCode(id, variation.Id, incoming.Code.Trim(), incoming.Value);
                    continue;
                }

                current.Value = incoming.Value;
                current.CustomValue = incoming.CustomValue;
                current.Unit = incoming.Unit;
                if (incoming.Code == "model")
                    variation.Model = incoming.Value;
                if (incoming.Code == "article")
                    variation.Article = incoming.Value;
            }
        }
